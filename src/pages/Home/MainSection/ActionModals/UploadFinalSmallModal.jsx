...
const UploadFinalSmallModal = ({ toggleModal , fileData , globalMetadata , fileMetadata }) => {
   React.useEffect(() => {
      disableBodyScroll();
      return () => {
         enableBodyScroll();
   ...
   async function handleUpload(event) {
         ...
         let orderfilenames = fileData.map((item)=>{ return item.file.name}); 
         ...
         fileData.forEach(async (eachfile,index)=>{
            try {
                  ...
                  const metadata = {contentType:'application/pdf',customMetadata:{orderIdNo,...globalMetadata,...fileMetadata},uuid:uid};
                  
                  await uploadBytesResumable(storageref2,eachfile.file,metadata);
         ...
         >
            ...
            <div className="modal-container-inner">
               ...
               <div className="modal-footer">
                  <button
                     onClick={(e)=>{
                        if(proceedOption==='Upload and Print Later')handleUpload(e)
                        if (proceedOption === "Print and Store") {
                           handleUpload(e).then(() => {
                              const orderIdNo = Object.keys(orderIdArray)[0];
                              const userId = localStorage.getItem("userid");
                              const filenames = orderIdArray[orderIdNo];
                              const metadata = { ...globalMetadata, ...fileMetadata };
               
                              sendRequestToBackend(orderIdNo, userId, filenames, metadata);
                           });
                        }
                     }}
                  >
